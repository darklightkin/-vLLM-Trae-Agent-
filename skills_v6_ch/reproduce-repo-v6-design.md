...
`skills_v5` 已经有了正式 skill 的骨架，比 `autoresearch/program.md` 更规范。v6 的改进重点不是继续堆文字，而是把它从“复现说明书”升级成一个可执行、可审计、可迭代的复现 Agent 协议。
...
- references 拆得合理：环境、指标、低交互、仓库扫描分开。
...
现在 workflow 是线性列表，但复现任务经常会失败、回退、分叉。需要把它设计成状态机：
...
这不是最终提交物，而是 Agent 判断“我现在是否真的完成了”的依据。
...
已经有 raw trace rule，但现实里 Agent 很容易把 history 写成总结报告。建议增加明确判断：
...
复现任务不像 `autoresearch` 可以无限跑。应该定义 bounded repair：
...
细节、例子、异常处理、历史经验，不要都塞进 `SKILL.md`，应放到 references 或 scripts。
...
v5 已经是“规则型 skill”，v6 应该升级成“状态机 + 验证器 + 审计文件”的复现系统。
