# gpu-and-checkpoint-handling v2.1
...
- If only one checkpoint is available but the paper reports multi-fold results, record the limitation and do not claim full multi-fold reproduction.
...
- If a dataset lacks explicit system/model IDs needed for system-level metrics, inspect official filenames, metadata, README, or analysis scripts before inventing a grouping field.
- Record split semantics. If a run combines train/validation/test, classify it as diagnostic unless the official protocol says to do so.
