# Skills v2.1 Design Notes

This folder contains the English low-interaction reproduction skills. Version 2.1 strengthens submission discipline based on the MuQ-Eval `history_v1` review: clean history generation, failure recovery tables, task-card-driven result filenames, run-type separation, and explicit protocol deviation records.
...
5. Generate the task-card requested table file and `conversation_history/history.md`.
6. Separate `smoke`, `diagnostic`, and `submission` runs. Only submission runs may feed the final table.
7. Add a failure recovery table and protocol deviation section to history.
```
...
The final table file contains only the result table. Commands, errors, fixes, run type, protocol deviations, data sources, and interaction counts go into history.

## Required Clean History

Raw JSONL or terminal logs are useful evidence, but they are not enough as final conversation history. Each run should produce:

```text
conversation_history/history.md
```

with sections for task, skills used, environment, resources, commands, smoke runs, diagnostic runs, submission run, failure recovery table, metrics, protocol deviations, files produced, and interaction count.
